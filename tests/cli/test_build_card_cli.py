...
class TestBuildCardOrchestrator:
    ...
    def test_module_names_for_fixtures_returns_all_registry_signals(self):
        from alpha_agent.cli.build_card import _module_names_for_fixtures

        names = _module_names_for_fixtures()
        # Now derived from the signal registry (source of truth). 14 signals:
        # the prior 13 + geopolitical_impact, which the hand-kept list had
        # silently dropped (the exact drift this consolidation fixes).
        assert len(names) == 14
        assert "factor" in names
        assert "rsrs" in names
        assert "calendar" in names
        assert "political_impact" in names
        assert "geopolitical_impact" in names
