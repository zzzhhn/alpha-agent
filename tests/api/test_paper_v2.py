...
async def _seed_pick_run(applied_db, ticker: str, market_date: date) -> int:
    conn = await asyncpg.connect(applied_db)
    try:
        run_id = await conn.fetchval(
            "INSERT INTO research_run "
            "(scheduled_for_date, run_type, status, started_at, finished_at, "
            "weight_policy_id, health_json) "
            "VALUES ($1, 'daily_close', 'complete', now(), now(), "
            "'policy-test', '{\"passed\":true}'::jsonb) RETURNING id",
            market_date,
        )
        await conn.execute(
            "INSERT INTO rating_snapshot "
            "(run_id, ticker, eligible, rank, tier, user_visible_payload_json) "
            "VALUES ($1, $2, true, 1, 'BUY', $3::jsonb)",
            run_id,
            ticker,
            '{"ticker":"' + ticker + '","rating":"BUY"}',
        )
        return int(run_id)
    finally:
        await conn.close()
...
async def test_buy_rejected_when_cash_insufficient(client_with_db, applied_db):
    ...
    assert "2,002.00" in detail or "2002.00" in detail


async def test_buy_allowed_at_exact_cash_boundary(client_with_db, applied_db):
    user_id, _ = await _seed_user_and_account(applied_db, cash=2002.0)
    await _seed_close(applied_db, "AAPL", 200.0)
...
async def test_buy_rejected_when_no_price_data(client_with_db, applied_db):
    """A market order without a trusted close cannot reserve cash honestly."""
...
def test_pick_attribution_round_trips_through_order_and_list(client_with_db, applied_db):
    ...
    run_id = asyncio.run(_seed_pick_run(applied_db, "AAPL", pick_d))
    asyncio.run(_seed_close(applied_db, "AAPL", 200.0, pick_d))
    ...
    assert orders[0]["source_run_id"] == run_id
    assert orders[0]["source_policy_id"] == "policy-test"
    assert len(orders[0]["source_payload_hash"]) == 64
...
def test_manual_order_has_null_pick_fields(client_with_db, applied_db):
    ...
    user_id, _ = asyncio.run(_seed_user_and_account(applied_db))
    asyncio.run(_seed_close(applied_db, "AAPL", 200.0))
    r = client_with_db.post(
        "/api/paper/order",
        headers=_auth(user_id),
...
def test_pending_buys_reserve_cash_before_fill(client_with_db, applied_db):
    import asyncio

    import alpha_agent.storage.postgres as _pg

    user_id, _ = asyncio.run(_seed_user_and_account(applied_db, cash=1000.0))
    asyncio.run(_seed_close(applied_db, "AAPL", 100.0))
    first = client_with_db.post(
        "/api/paper/order",
        headers=_auth(user_id),
        json={"ticker": "AAPL", "side": "buy", "order_type": "market", "qty": 6},
    )
    ...
    second = client_with_db.post(
        "/api/paper/order",
        headers=_auth(user_id),
        json={"ticker": "AAPL", "side": "buy", "order_type": "market", "qty": 5},
    )
    assert second.status_code == 400

    _pg._pool = None
    _pg._pool_dsn = None
    account = client_with_db.get("/api/paper/account", headers=_auth(user_id)).json()
    assert account["reserved_cash"] == pytest.approx(600.60)
    assert account["available_cash"] == pytest.approx(399.40)


async def test_reset_opens_new_cohort_without_rewriting_history(
    client_with_db, applied_db
):
    user_id, account_id = await _seed_user_and_account(applied_db, cash=900.0)
    conn = await asyncpg.connect(applied_db)
    try:
        await conn.execute(
            "INSERT INTO sim_position "
            "(account_id, cohort_id, ticker, qty, avg_cost) "
            "VALUES ($1, 0, 'AAPL', 2, 100.0)",
            account_id,
        )
        await conn.execute(
            "INSERT INTO sim_order "
            "(account_id, cohort_id, ticker, side, order_type, qty, signal_date) "
            "VALUES ($1, 0, 'AAPL', 'buy', 'market', 1, $2)",
            account_id,
            date.today(),
        )
    finally:
    ...
    response = client_with_db.post("/api/paper/reset", headers=_auth(user_id))
    assert response.status_code == 200
    ...
    try:
        assert await conn.fetchval(
            ...
            "WHERE account_id=$1 AND cohort_id=0 AND ticker='AAPL'",
            account_id,
        ) == 2
        assert await conn.fetchval(
            "SELECT status FROM sim_order WHERE account_id=$1 AND cohort_id=0",
            account_id,
        ) == "cancelled"
        assert await conn.fetchval(
            "SELECT reset_count FROM sim_account WHERE id=$1", account_id
        ) == 1
    finally:
        await conn.close()
...
async def test_fill_marks_order_failed_when_batch_exhausts_cash(applied_db):
    ...
    try:
        ...
        assert "600.60" in failed_row["fail_reason"]

        acct = await conn.fetchrow("SELECT cash FROM sim_account WHERE id = $1", acct_id)
        assert acct["cash"] == pytest.approx(1000.0 - 600.60)
    finally:
        await conn.close()
