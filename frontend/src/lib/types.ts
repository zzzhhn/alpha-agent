...
export interface IcTrendSeries {
  ...
  readonly points: readonly IcTrendPoint[];
}

export interface IcTrendResponse {
  readonly window_days: number;
  readonly series: readonly IcTrendSeries[];
}

export interface EvolutionWeight {
  readonly signal_name: string;
  readonly status: "live" | "shadow";
  readonly weight: number;
  readonly reason: string | null;
  readonly consecutive_bad_windows: number;
  readonly shadow_streak: number;
  readonly last_updated: string | null;
}

export interface EvolutionWeightsResponse {
  readonly weights: readonly EvolutionWeight[];
}
...
export interface EvolutionCalibration {
  ...
  readonly isotonic_map?: { readonly x: readonly number[]; readonly y: readonly number[] };
