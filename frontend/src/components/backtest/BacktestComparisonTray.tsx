  ...
  return (
    <div className={`min-w-0 border px-4 py-3 ${active ? "border-tm-accent bg-tm-accent/5" : "border-tm-rule bg-tm-bg"}`}>
      <div className="flex items-center justify-between gap-3">
        <p className={`text-xs uppercase tracking-[0.08em] ${active ? "text-tm-accent" : "text-tm-muted"}`}>{label}</p>
        {run ? <span className="text-xs text-tm-muted">#{run.id.slice(-6)}</span> : null}
      </div>
      {run ? (
        <>
          <p className="mt-2 truncate font-mono text-xs text-tm-fg" title={run.params.expression}>{run.params.expression}</p>
          <p className="mt-2 font-mono text-xs text-tm-fg-2">
        ...
        <p className="mt-2 text-xs text-tm-muted">{zh ? "尚无可比较运行" : "No comparable run yet"}</p>
      )}
    </div>
  );
}
...
export function BacktestComparisonTray({
  ...
  return (
    <section className="mt-3 grid grid-cols-[140px_repeat(3,minmax(0,1fr))] gap-2" aria-label={zh ? "运行比较托盘" : "Run comparison tray"}>
      <div className="flex items-center border border-tm-rule bg-tm-bg-2 px-4 text-xs font-semibold text-tm-fg-2">
        {zh ? "对比摘要" : "Comparison"}
      </div>
