...
export default function AlertWorkbench() {
  ...
  return (
    <TmScreen className="min-h-[calc(100vh-36px)] font-tm-mono text-tm-fg">
      ...
      <div className="flex flex-1 flex-col">
        <div className="mx-6 mt-4 grid min-h-[600px] grid-cols-1 overflow-hidden border border-tm-rule min-[1280px]:grid-cols-[0.9fr_2.2fr_1.25fr]">
          <aside className="border-r border-tm-rule bg-tm-bg-2/50 p-3">
            <p className="px-2 py-1 text-xs uppercase tracking-[0.12em] text-tm-muted">
              ...
              {QUEUES.map(({ id, icon: Icon }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="border border-tm-rule px-1.5 py-0.5 text-xs">{queueCount(data, id)}</span>
                  </TmRowButton>
                );
              })}
            </nav>
            ...
            <div className="mt-5 border-t border-tm-rule pt-3">
              <p className="px-2 text-xs uppercase tracking-[0.12em] text-tm-muted">
                {zh ? "严重性" : "Severity"}
              </p>
            ...
            <div className="mt-5 border-t border-tm-rule pt-3">
              <p className="px-2 text-xs uppercase tracking-[0.12em] text-tm-muted">{zh ? "关联对象" : "Relevance"}</p>
            ...
            <div className="mt-5 border-t border-tm-rule px-2 pt-3 text-xs leading-5 text-tm-muted">
              ...
              <p className="mt-1 text-xs">{zh ? "筛选配置仅保存在当前浏览器" : "Filters are browser-local"}</p>
            </div>
          </aside>

          <main className="min-w-0 border-r border-tm-rule">
            <div className="flex h-11 items-center justify-between border-b border-tm-rule bg-tm-bg-2/40 px-4 text-xs">
            ...
            <div className="grid h-9 grid-cols-[68px_88px_minmax(150px,1.2fr)_minmax(130px,1fr)_84px] items-center gap-2 border-b border-tm-rule bg-tm-bg px-4 text-xs uppercase tracking-[0.08em] text-tm-muted">
            ...
            {error && data ? (
              <div role="alert" className="flex items-center justify-between gap-3 border-b border-tm-neg/50 bg-tm-neg/5 px-4 py-2 text-xs">
            ...
            {authRequired ? (
              <div className="flex min-h-[360px] items-center justify-center border-b border-tm-rule bg-tm-bg-2/30 p-6 text-center">
                <div className="max-w-md">
                  ...
                  <p className="mt-2 text-xs leading-5 text-tm-muted">
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <p className="mt-3 text-xs text-tm-muted">{relativeTime(item.created_at, locale)}</p>
                        {item.state.status === "snoozed" ? (
                          <p className="mt-1 text-xs text-tm-warn">{zh ? "已暂缓" : "Snoozed"}</p>
                        ) : null}
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="mt-2 text-xs text-tm-fg-2">
                        ...
                        <p className="mt-1 text-xs text-tm-accent">{zh ? "查看变化详情 →" : "View change details →"}</p>
                      </div>
                      <div>
                        <p className="text-xs leading-5 text-tm-fg-2">{impactSummary(item, locale)}</p>
                        <p className="mt-2 text-xs text-tm-muted">{relevanceLabel(item.relevance, locale)}</p>
                      </div>
          ...
          <aside className="min-w-0 bg-tm-bg-2/20">
            {!selected ? (
              <div className="flex h-full items-center justify-center p-6 text-center text-xs text-tm-muted">
                {zh ? "选择一条警报查看证据和关联对象。" : "Select an alert to inspect its evidence and linked objects."}
              </div>
            ) : (
              <div className="h-full overflow-y-auto">
                <div className="flex h-11 items-center justify-between border-b border-tm-rule px-4 text-xs">
                ...
                <section className="border-b border-r border-tm-rule p-4 min-[1680px]:border-r-0">
                  <h2 className="text-xs font-semibold uppercase tracking-[0.08em] text-tm-accent">
                  ...
                  <div className="mt-4 border border-tm-rule bg-tm-bg px-3 py-3">
                    <p className="text-xs uppercase tracking-[0.1em] text-tm-muted">{zh ? "证据时间窗" : "Evidence window"}</p>
                    <div className="relative mt-3 flex items-start justify-between before:absolute before:left-2 before:right-2 before:top-1.5 before:h-px before:bg-tm-rule">
                      {[
                        ...
                        <div key={`${label}-${index}`} className="relative z-10 w-1/3 text-center">
                          ...
                          <p className="mt-2 text-xs text-tm-fg-2">{label}</p>
                          <p className="mt-0.5 text-xs text-tm-muted">{stamp ? new Date(stamp).toLocaleTimeString(zh ? "zh-CN" : "en-US", { hour: "2-digit", minute: "2-digit" }) : "—"}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <section className="border-b border-r border-tm-rule p-4 min-[1680px]:border-r-0">
                  ...
                  <p className="mt-2 text-xs leading-5 text-tm-fg-2">{impactSummary(selected, locale)}</p>
                  <div className="mt-3 grid grid-cols-2 gap-px bg-tm-rule border border-tm-rule text-xs">
                ...
                <section className="p-4">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2">
                    ...
                    <Link
                      ...
                      className="flex items-center justify-center gap-1.5 border border-tm-rule px-2 py-1.5 text-xs text-tm-fg-2 hover:border-tm-accent hover:text-tm-accent"
                    >
                      <ExternalLink className="h-3 w-3" /> {zh ? "打开研究" : "Open research"}
                    </Link>
                  </div>
                  <div className="mt-4 border-t border-tm-rule pt-3 text-xs leading-5 text-tm-muted">
        ...
        <section className="mx-6 mb-4 border-x border-b border-tm-rule">
          <div className="flex min-h-8 items-center justify-between border-b border-tm-rule bg-tm-bg-2/40 px-4 text-xs uppercase tracking-[0.08em] text-tm-muted">
          ...
          <div className="grid h-9 grid-cols-[190px_100px_120px_minmax(0,1fr)_120px] items-center gap-3 border-b border-tm-rule bg-tm-bg-2/40 px-4 text-xs uppercase tracking-[0.08em] text-tm-muted">
          ...
          {auditItems.length > 0 ? auditItems.map((item) => (
            <div key={`audit-${item.id}`} className="grid min-h-9 grid-cols-[190px_100px_120px_minmax(0,1fr)_120px] items-center gap-3 border-b border-tm-rule px-4 text-xs last:border-b-0">
            ...
            <div className="flex min-h-12 items-center px-4 text-xs text-tm-muted">
      ...
      <footer className="flex min-h-7 items-center justify-between border-t border-tm-rule px-3 text-xs text-tm-muted">
