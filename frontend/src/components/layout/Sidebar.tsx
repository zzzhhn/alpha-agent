  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 border-b border-[var(--border)] px-5 py-4">
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-[var(--accent)] to-[var(--accent-hover)] text-[13px] font-bold text-white">
          AC
        </div>
        <span className="text-base font-semibold text-[var(--text)]">
          {t(locale, "brand.name")}
        </span>
        <span className="ml-1 rounded bg-[var(--accent-glow)] px-1.5 py-px text-[10px] text-[var(--accent)]">
          {t(locale, "brand.tag")}
        </span>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto">
        {NAV_GROUPS.map((group) => (
          <div key={group.labelKey} className="px-3 pt-5 pb-1">
            <div className="mb-1.5 px-2 text-[11px] font-medium uppercase tracking-[0.08em] text-[var(--muted)]">
            ...
            {group.items.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={clsx(
                    "relative mb-0.5 flex items-center gap-2.5 rounded-lg border-l-[3px] px-3 py-2 text-[13px] transition-all duration-150",
                    isActive
                ...
                >
                  ...
                  {item.badge && (
                    <span className="ml-auto rounded bg-[var(--accent-glow)] px-1.5 py-px text-[10px] text-[var(--accent)]">
                      {item.badge}
                    </span>
                  )}
                </Link>
