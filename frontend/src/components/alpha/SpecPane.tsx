    ...
    <section className="flex min-h-[260px] flex-col gap-3 border border-tm-rule bg-tm-bg-2 p-4">
      ...
      {state === "waiting" ? (
        ...
        <>
          ...
          <div>
            <p className="mb-1.5 font-tm-mono text-[9px] uppercase tracking-[0.1em] text-tm-muted">{t(locale, "alpha.pane.operators" as Parameters<typeof t>[1])}</p>
            <div className="flex flex-wrap gap-1">
              {data.spec.operators_used.map((operator) => <span key={operator} className="border border-tm-rule px-2 py-1 font-mono text-[10px] text-tm-fg-2">{operator}</span>)}
            </div>
          </div>
          <div>
            <p className="mb-1.5 font-tm-mono text-[9px] uppercase tracking-[0.1em] text-tm-muted">{zh ? "输入字段" : "Input fields"}</p>
            <div className="divide-y divide-tm-rule border-y border-tm-rule">
              {operands.length > 0 ? operands.map((operand) => (
                <div key={operand} className="flex min-h-8 items-center justify-between text-[10px]">
                  <code className="text-tm-fg">{operand}</code>
                  <span className="text-tm-muted">{zh ? "服务端字段白名单" : "server field allowlist"}</span>
                </div>
              )) : <p className="py-2 text-[10px] text-tm-muted">{zh ? "表达式不含数据字段" : "No data operand in expression"}</p>}
            </div>
          </div>
          ...
          <div className="border-t border-tm-rule pt-2 text-[9.5px] leading-5 text-tm-muted">
