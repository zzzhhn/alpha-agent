  ...
  return (
    <div className="border-t border-tm-rule px-3 py-3">
      <div className="mb-3 rounded border border-tm-rule bg-tm-bg-2 px-3 py-2">
        <div className="font-tm-mono text-[10px] uppercase tracking-wide text-tm-muted">{locale === "zh" ? "连续份额级验证账户" : "Continuous share-level validation book"}</div>
        {continuous ? (
        ...
        ) : <p className="mt-2 font-tm-mono text-[10px] text-tm-muted">{locale === "zh" ? "等待迁移后初始化，只从部署后的新推荐开始，绝不回填已知历史价格。" : "Awaiting initialization. It starts only from a new post-deployment recommendation and never backfills already-known prices."}</p>}
        {continuous?.status === "awaiting_forward_run" ? <p className="mt-2 font-tm-mono text-[9.5px] leading-4 text-tm-muted">{locale === "zh" ? `已建立前瞻边界 RUN #${continuous.start_after_run_id}，等待下一份完整推荐。` : `Forward boundary is RUN #${continuous.start_after_run_id}; waiting for the next complete recommendation.`}</p> : null}
        {strategic ? <p className="mt-2 border-t border-tm-rule pt-2 font-tm-mono text-[9.5px] leading-4 text-tm-muted">{locale === "zh" ? `战略 60 日独立账户：${strategic.status === "active" ? `${strategic.positions} 个持仓，收益 ${pct(strategic.cumulative_return)}` : `已在 RUN #${strategic.start_after_run_id} 建立边界，等待同政策新快照`}` : `Strategic 60d independent book: ${strategic.status === "active" ? `${strategic.positions} positions, ${pct(strategic.cumulative_return)}` : `forward boundary at RUN #${strategic.start_after_run_id}`}`}</p> : null}
      </div>
      {summary.status !== "ready" ? <p className="font-tm-mono text-[10px] text-tm-muted">{t(locale, "sim.l2.accumulating")}</p> : <>
